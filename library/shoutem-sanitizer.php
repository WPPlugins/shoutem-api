	...
	if(preg_match_all('/<img.*?>/i',$html,$matches) > 0) {
		foreach($matches[0] as $index => $imageTag) {
			...
			if ($image[width]==='' && $image[height]===''){
				...
				//first try to fetch image size locally (faster)
				$imagelocation = str_replace(get_site_url(),'',$image[src]);
				$imagelocation = ltrim($imagelocation, "/");
				$size = @getimagesize($imagelocation);
				
				//failover for remote fetch
				if(!$size){
